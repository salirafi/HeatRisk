...
from src.helpers import write_runtime_metadata
...
def build_runtime_metadata(df: pd.DataFrame) -> dict[str, Any]:
    if df.empty:
        return {}

    fetched_at = pd.to_datetime(df["fetched_at"].iloc[0], errors="coerce")
    forecast_times = (
        pd.to_datetime(df["local_datetime"], errors="coerce")
        .dropna()
        .sort_values()
        .drop_duplicates()
        .tolist()
    ...
    return {
        "last_db_update": None if pd.isna(fetched_at) else pd.Timestamp(fetched_at).isoformat(),
...
def save_forecasts(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        conn.close()
    write_runtime_metadata(build_runtime_metadata(df))
